...
import {ChevronDown, ChevronLeft, ChevronRight, Eye, EyeOff, Info, Settings2, SlidersHorizontal} from "lucide-react";
...
export const NodeConfigForm: React.FC<Props> = ({node, onChange, isOpen, setIsOpen}) => {
    ...
    const isPropDisabled = (prop: IInputProp) => {
        if (prop.disabled) return true;
        if (!prop.dependOn) return false;

        const approvalProp = node.data.inputProps.find(p => p.type === "approval");

        if (!approvalProp || !Array.isArray(approvalProp.value)) {
            return true;
        }

        const isServiceEnabled = approvalProp.value.some(
            (item: any) => item.service === prop.dependOn
        );
    ...
    return (
        <>
            ...
            >
                ...
                <div className="flex-1 p-6 overflow-y-auto scrollbar-thin scrollbar-thumb-slate-800 scrollbar-track-transparent custom-scrollbar">
                    {!node ? (
                        ...
                        <div className="space-y-6 pb-20">
                            {node.data.inputProps.map((prop: IInputProp) => {
                                const isDisabled = isPropDisabled(prop);

                                switch (prop?.type) {
                                    case "text":
                                        return (
                                            <div key={prop.name} className={cn("space-y-2 transition-all duration-200", isDisabled && "opacity-20 grayscale pointer-events-none")}>
                                                <label className="text-[10px] font-bold uppercase tracking-widest text-slate-500 ml-1">{prop.displayName}</label>
                                                <input
                                                    disabled={isDisabled}
                                                    className="w-full rounded-md border border-slate-800 bg-slate-900/50 px-3 py-2 text-sm text-white placeholder:text-slate-700 focus:outline-none focus:ring-1 focus:ring-emerald-500/40"
                                                    value={prop.value ?? ""}
                                    ...
                                    case "password":
                                        const isVisible = !!visiblePasswords[prop.name];
                                        return (
                                            <div key={prop.name} className={cn("space-y-2 transition-all duration-200", isDisabled && "opacity-20 grayscale pointer-events-none")}>
                                                <label className="text-[10px] font-bold uppercase tracking-widest text-slate-500 ml-1">{prop.displayName}</label>
                                                <div className="relative">
                                                    <input
                                                        ...
                                                        disabled={isDisabled}
                                                        className="w-full rounded-md border border-slate-800 bg-slate-900/50 px-3 py-2 pr-10 text-sm text-white placeholder:text-slate-700 focus:outline-none focus:ring-1 focus:ring-emerald-500/40"
                                                        value={prop.value ?? ""}
                                                        onChange={(e) => onChange(prop.name, e.target.value)}
                                                    />
                                                    <button
                                        ...
                                        return (
                                            <div key={prop.name} className={cn("space-y-2 transition-all duration-200", isDisabled && "opacity-20 grayscale pointer-events-none")}>
                                                ...
                                                <div className="relative">
                                                    <select
                                                        disabled={isDisabled}
                                                        className="w-full rounded-md border border-slate-800 bg-slate-900/50 px-3 py-2 text-sm text-white appearance-none cursor-pointer focus:outline-none"
                                                        value={prop.value ?? ""}
                                                        onChange={(e) => onChange(prop.name, e.target.value)}
                                                    >
                                                        {prop.values?.map((v: any) => (
                                                            <option key={v.value} value={v.value} className="bg-slate-900">{v.name}</option>
                                                        ))}
                                                    </select>
