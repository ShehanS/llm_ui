...
import {AlertCircle, ChevronDown, ChevronUp, Terminal, X, Plus} from "lucide-react"; // Added Plus icon
...
const Page: FC = () => {
    ...
    const { openCommonDialog } = useDialogDataStore();
    ...
    // --- LOGIC TO ADD HANDLER AT RUNTIME ---
            ...
            nds.map((node) => {
                if (node.id === selectedNodeId) {
                    const newOutput = {
                        ...
                        label: `Output ${ (node.data.outputs?.length || 0) + 1}`,
                        type: 'source'
                    };
            ...
            const newNode = {
                ...
                data: {
                    ...
                    inputProps: catalogNode.config?.inputProps
                        ...
                        : [],
                    outputs: catalogNode.outputs || [] // Initialize with catalog outputs
                },
            };
    ...
    const updateNodeConfig = (propName: any, value: any) => {
        setNodes((nds) =>
            nds.map((node) => {
                ...
                const updatedInputProps = node.data.inputProps.map((p: any) => {
                    if (propName === "logic" && p.name === "logic") {
                        return {
                            ...p,
                            value: value,
                        };
                    ...
                    if (isMapper && p.type === "mapper") {
                        return {
                            ...p,
                            value: {
                                ...(typeof p.value === "object" && !Array.isArray(p.value) ? p.value : {}),
                                [propName.type]: value,
                            },
                        };
                    }
                ...
                return {
                    ...
                    data: {
                        ...
                        outputs: propName === "logic" ? value : (node.data.outputs || []),
                    },
                };
            })
        );
    ...
    useEffect(() => {
        if (workflow) {
            const loadedNodes = workflow.definition?.nodes.map((n: any) => {
                const inputProps = n.config?.inputProps ?? [];
                const logicProp = inputProps.find((p: any) => p.type === "logic");

                const syncOutputs = (logicProp && Array.isArray(logicProp.value) && logicProp.value.length > 0)
                    ? logicProp.value
                    : (n.outputs ?? []);
    ...
    return (
        <div className="h-screen w-full bg-[#0f172a] flex overflow-hidden font-sans text-slate-200">
            {loading && <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 backdrop-blur-sm"><Spinner/></div>}

            <OpenWorkflowDialog open={openDialog} onClose={() => setOpenDialog(false)} files={workflows} onOpenFile={(id) => { openWorkflow(id); setOpenDialog(false); }}/>
            <SaveWorkflowDialog open={saveDialog} fileName={workflow?.flowName ?? ""} description={workflow?.description ?? ""} onClose={() => setSaveDialog(false)}
                                onSave={async (fileName, description) => {
                                    const wfDef = {
                                        nodes: nodes.map((n) => ({
                                            ...
                                            color: n.data.color,
                                            position: n.position,
                                            config: { icon: n.data.config?.icon ?? "", inputProps: n.data.inputProps ?? [] },
                                    ...
                                    const payload = { flowId: workflow?.flowId || getId(), flowName: fileName, description, definition: wfDef, state: true };
            ...
            <div className="flex-1 flex flex-col relative overflow-hidden transition-all duration-300 ease-in-out bg-slate-900/50" style={{ marginLeft: leftOpen ? "260px" : "0px", marginRight: rightOpen ? "320px" : "0px" }}>
                ...
                <div className="absolute top-6 left-1/2 -translate-x-1/2 z-50 flex gap-2 bg-slate-900/95 p-2 rounded-xl border border-slate-700 backdrop-blur-md shadow-2xl">
                    ...
                    <button
                        ...
                        className={cn(
                            "px-4 py-2 rounded-lg text-xs font-semibold flex items-center gap-2 transition-colors",
                            selectedNodeId ? "bg-indigo-600 hover:bg-indigo-500 text-white" : "bg-slate-800 text-slate-500 cursor-not-allowed"
                        )}
                    >
                        <Plus size={14} /> Add Output
                    </button>

                    <button onClick={() => setSaveDialog(true)} className="px-4 py-2 bg-slate-800 hover:bg-slate-700 rounded-lg text-xs font-semibold">Save</button>
                    <button onClick={async () => { await openWorkflows(); setOpenDialog(true); }} className="px-4 py-2 bg-slate-800 hover:bg-slate-700 rounded-lg text-xs font-semibold">Open</button>
                    ...
                    {!traceConnected ? (
                        ...
                        <button onClick={stopLiveTrace} className="px-5 py-2 bg-red-600 rounded-lg text-xs font-semibold flex items-center gap-2">
                            <div className="w-2 h-2 rounded-full bg-white animate-ping"/> Stop Trace
                        </button>
                    )}
                </div>
                    ...
                    <div className={cn("absolute bottom-4 left-4 right-4 bg-slate-950 border border-slate-800 rounded-xl z-[60] transition-all duration-300 shadow-2xl overflow-hidden flex flex-col", isPanelExpanded ? 'h-[70vh]' : 'h-10')}>
                        <div className="flex items-center justify-between px-4 h-10 bg-slate-900 border-b border-slate-800 cursor-pointer shrink-0" onClick={() => setIsPanelExpanded(!isPanelExpanded)}>
                            <div className="flex items-center gap-2">
                                <Terminal size={14} className={cn(debugDetails.status === 'FAILED' ? "text-red-400" : debugDetails.status === 'WAITING' ? "text-indigo-400" : "text-emerald-400")}/>
                                <span className="text-[10px] font-bold uppercase tracking-widest text-slate-200">Debug Console: {debugDetails.nodeType} ({debugDetails.status})</span>
                            </div>
                            <div className="flex items-center gap-4">
                                <button className="text-slate-400 hover:text-white">{isPanelExpanded ? <ChevronDown size={18}/> : <ChevronUp size={18}/>}</button>
                                <button onClick={(e) => { e.stopPropagation(); setDebugDetails(null); }} className="text-slate-400 hover:text-red-400"><X size={18}/></button>
                            </div>
                        </div>
                        {isPanelExpanded && (
                            <div className="flex-1 flex flex-col min-h-0 bg-slate-950 p-4">
                                ...
                                <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                                    <section className="flex flex-col space-y-2">
                                        ...
                                        <pre className="bg-slate-900/50 p-3 rounded-xl border border-slate-800/50 overflow-auto text-[11px] font-mono text-slate-300 max-h-[400px]">{JSON.stringify(debugDetails.config, null, 2)}</pre>
                                    </section>
                                    <section className="flex flex-col space-y-2">
                                        <div className="flex items-center gap-2"><div className="w-1 h-3 bg-blue-500 rounded-full"/><span className="text-[10px] text-blue-400 font-bold uppercase">Input</span></div>
                                        <pre className="bg-slate-900/50 p-3 rounded-xl border border-slate-800/50 overflow-auto text-[11px] font-mono text-slate-300 max-h-[400px]">{JSON.stringify(debugDetails.input, null, 2)}</pre>
                                    </section>
                                    <section className="flex flex-col space-y-2">
                                        <div className="flex items-center gap-2"><div className="w-1 h-3 bg-emerald-500 rounded-full"/><span className="text-[10px] text-emerald-400 font-bold uppercase">Output</span></div>
                                        <pre className="bg-slate-900/50 p-3 rounded-xl border border-slate-800/50 overflow-auto text-[11px] font-mono text-slate-300 max-h-[400px]">{JSON.stringify(debugDetails.output, null, 2) || "null"}</pre>
                                    </section>
                                </div>
                            </div>
                        )}
            ...
            <NodeConfigForm node={selectedNode} onChange={updateNodeConfig} isOpen={rightOpen} setIsOpen={setRightOpen}/>
        </div>
    );
};
